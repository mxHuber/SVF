...
SVFIR* SVFIRBuilder::build()
{
    ...
    // We read SVFIR from a user-defined txt instead of parsing SVFIR from LLVM
    // IR
    ...
    // If the SVFIR has been built before, then we return the unique SVFIR of
    // the program
    ...
    for (Module& M : LLVMModuleSet::getLLVMModuleSet()->getLLVMModules())
    {
        for (Module::const_iterator F = M.begin(), E = M.end(); F != E; ++F)
        {
            ...
            {
                /// Return SVFIR node will not be created for function which can
                /// not reach the return instruction due to call to abort(),
                /// exit(), etc. In 176.gcc of SPEC 2000, function
                /// build_objc_string() from c-lang.c shows an example when
                /// fun.doesNotReturn() evaluates to TRUE because of abort().
                ...
                /// commandline?
                for (Function::const_arg_iterator I = fun.arg_begin(),
                                                  E = fun.arg_end();
                     I != E; ++I)
                {
                    setCurrentLocation(&*I, &fun.getEntryBlock());
                    NodeID argValNodeId = pag->getValueNode(
                        LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&*I));
                    // if this is the function does not have caller (e.g. main)
                    // or a dead function, shall we create a black hole address
                    // edge for it? it is (1) too conservative, and (2) make
...
bool SVFIRBuilder::computeGepOffset(const User* V, AccessPath& ap)
{
         ...
         gi != ge; ++gi)
    {
        ...
        // if Options::ModelConsts() is disabled. We will treat whole array as
        // one, but we can distinguish different field of an array of struct,
        // e.g. s[1].f1 is different from s[0].f2
        if (const ArrayType* arrTy = SVFUtil::dyn_cast<ArrayType>(gepTy))
        {
            if (!op ||
                (arrTy->getArrayNumElements() <= (u32_t)op->getSExtValue()))
                continue;
            APOffset idx = op->getSExtValue();
            u32_t offset = pag->getSymbolInfo()->getFlattenedElemIdx(
                LLVMModuleSet::getLLVMModuleSet()->getSVFType(arrTy), idx);
        ...
        else if (const StructType* ST = SVFUtil::dyn_cast<StructType>(gepTy))
        {
            ...
            APOffset idx = op->getSExtValue();
            u32_t offset = pag->getSymbolInfo()->getFlattenedElemIdx(
                LLVMModuleSet::getLLVMModuleSet()->getSVFType(ST), idx);
            ap.setFldIdx(ap.getConstantStructFldIdx() + offset);
        }
        else if (gepTy->isSingleValueType())
        {
            ...
            // accessing (%result = gep %struct.A* %a, i32 %non-const-index) If
            // its point-to target is single value (pointer arithmetic), then
            // it's a variant gep (%result = gep i8* %p, i32 %non-const-index)
            ...
            // consider using
            // inferFieldIdxFromByteOffset(geopOp,dataLayout,ap,idx)?
            // ap.setFldIdx(ap.getConstantFieldIdx() +
...
void SVFIRBuilder::processCE(const Value* val)
{
    if (const Constant* ref = SVFUtil::dyn_cast<Constant>(val))
    {
        if (const ConstantExpr* gepce = isGepConstantExpr(ref))
        {
            ...
            AccessPath ap(0,
                          LLVMModuleSet::getLLVMModuleSet()->getSVFType(pType));
            ...
            addGepEdge(
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(gepce)),
                ap, constGep);
        ...
        else if (const ConstantExpr* castce = isCastConstantExpr(ref))
        {
            ...
            addCopyEdge(
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(castce)),
                CopyStmt::BITCAST);
            setCurrentLocation(cval, cbb);
        }
        else if (const ConstantExpr* selectce = isSelectConstantExpr(ref))
        {
            ...
            NodeID cond = pag->getValueNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFValue(
                    selectce->getOperand(0)));
            NodeID nsrc1 = pag->getValueNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFValue(src1));
            NodeID nsrc2 = pag->getValueNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFValue(src2));
            NodeID nres = pag->getValueNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFValue(selectce));
        ...
        else if (const ConstantExpr* int2Ptrce = isInt2PtrConstantExpr(ref))
        {
            ...
            addCopyEdge(
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(int2Ptrce)),
                CopyStmt::INTTOPTR);
            setCurrentLocation(cval, cbb);
        }
        else if (const ConstantExpr* ptr2Intce = isPtr2IntConstantExpr(ref))
        {
            ...
            addCopyEdge(
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                pag->getValueNode(
                    LLVMModuleSet::getLLVMModuleSet()->getSVFValue(ptr2Intce)),
                CopyStmt::PTRTOINT);
            setCurrentLocation(cval, cbb);
        }
        else if (isTruncConstantExpr(ref) || isCmpConstantExpr(ref))
        {
            ...
            NodeID dst = pag->getValueNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFValue(ref));
                                 ...
                                 u32_t offset)
{
    DBOUT(
        ...
        outs()
            ...
            << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(C)->toString()
            << "\n");
    if (C->getType()->isSingleValueType())
    {
        NodeID src = getValueNode(C);
        // get the field value if it is available, otherwise we create a dummy
        // field node.
        setCurrentLocation(gvar, nullptr);
        NodeID field = getGlobalVarField(
            gvar, offset,
            LLVMModuleSet::getLLVMModuleSet()->getSVFType(C->getType()));
    ...
    else if (ConstantData* data = SVFUtil::dyn_cast<ConstantData>(C))
    {
        if (Options::ModelConsts())
        {
                    ...
                    SVFUtil::dyn_cast<ConstantDataSequential>(data))
            {
                ...
                {
                    u32_t off = pag->getSymbolInfo()->getFlattenedElemIdx(
                        LLVMModuleSet::getLLVMModuleSet()->getSVFType(
                            C->getType()),
                        i);
...
void SVFIRBuilder::visitGlobal(SVFModule* svfModule)
{
    ...
    for (Module& M : LLVMModuleSet::getLLVMModuleSet()->getLLVMModules())
    {
             ...
             I != E; I++)
        {
            ...
            NodeID dst = pag->getValueNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFValue(alias));
...
void SVFIRBuilder::visitPHINode(PHINode& inst)
{
    ...
    for (u32_t i = 0; i < inst.getNumIncomingValues(); ++i)
    {
        ...
        (void)
            matched; // Suppress warning of unused variable under release build
        ...
        const SVFInstruction* svfPrevInst =
            LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(predInst);
...
void SVFIRBuilder::visitGetElementPtrInst(GetElementPtrInst& inst)
{
    ...
    AccessPath ap(0, LLVMModuleSet::getLLVMModuleSet()->getSVFType(
...
void SVFIRBuilder::visitCallSite(CallBase* cs)
{
    ...
    const SVFInstruction* svfcall =
        LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(cs);

    DBOUT(DPAGBuild, outs()
                         << "process callsite " << svfcall->toString() << "\n");
    ...
    for (u32_t i = 0; i < cs->arg_size(); i++)
        pag->addCallSiteArgs(callBlockNode,
                             pag->getGNode(getValueNode(cs->getArgOperand(i))));
...
void SVFIRBuilder::visitBranchInst(BranchInst& inst)
{
    ...
    for (const Instruction* succInst : nextInsts)
    {
        ...
        const SVFInstruction* svfSuccInst =
            LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(succInst);
...
void SVFIRBuilder::visitSwitchInst(SwitchInst& inst)
{
    ...
    for (const Instruction* succInst : nextInsts)
    {
        ...
        const ConstantInt* condVal =
            inst.findCaseDest(const_cast<BasicBlock*>(succInst->getParent()));
...
/// <val> For now, we assume <val> is never a poison or undef.
...
void SVFIRBuilder::handleDirectCall(CallBase* cs, const Function* F)
{
    ...
    DBOUT(DPAGBuild, outs() << "handle direct call " << svfcall->toString()
    ...
    if (!cs->getType()->isVoidTy())
    {
        ...
        FunExitICFGNode* exitICFGNode =
            pag->getICFG()->getFunExitICFGNode(svffun);
    ...
    for (; itF != ieF; ++itA, ++itF)
    {
        ...
        const Value *AA = cs->getArgOperand(itA),
                    *FA = &*itF; // current actual/formal arg
    ...
    if (F->isVarArg())
    {
        ...
        for (; itA != ieA; ++itA)
        {
            ...
            CallICFGNode* icfgNode = pag->getICFG()->getCallICFGNode(svfcall);
            FunEntryICFGNode* entry =
                pag->getICFG()->getFunEntryICFGNode(svffun);
...
void SVFIRBuilder::handleIndCall(CallBase* cs)
{
    const SVFInstruction* svfcall =
        LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(cs);
    const SVFValue* svfcalledval =
        LLVMModuleSet::getLLVMModuleSet()->getSVFValue(cs->getCalledOperand());
    ...
    for (; iter != eiter; iter++)
    {
             ...
             func_iter != functions.end(); func_iter++)
        {
            const Function* callee = SVFUtil::cast<Function>(
                LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(*func_iter));

            if (isExtCall(*func_iter))
            {
                setCurrentLocation(callee, callee->empty()
                                               ? nullptr
...
void SVFIRBuilder::sanityCheck()
{
    for (SVFIR::iterator nIter = pag->begin(); nIter != pag->end(); ++nIter)
    {
        ...
        //       sure some value has been store into this pointer before q =
        //       load p, some value should stored into p first like store w p;
 ...
 * table
 */
NodeID SVFIRBuilder::getGepValVar(const Value* val, const AccessPath& ap,
...
void SVFIRBuilder::setCurrentBBAndValueForPAGEdge(PAGEdge* edge)
{
    ...
    if (const SVFInstruction* curInst =
            SVFUtil::dyn_cast<SVFInstruction>(curVal))
    {
        ...
        {
            icfgNode =
                pag->getICFG()->getFunExitICFGNode(curInst->getFunction());
        }
        else
        {
    ...
    if (const CallPE* callPE = SVFUtil::dyn_cast<CallPE>(edge))
    {
        ...
        if (ICFGEdge* edge = pag->getICFG()->hasInterICFGEdge(
                callNode, entryNode, ICFGEdge::CallCF))
