 ...
 * Line 11->13 is removed due to a strong update at Line 13, which means Line 11
 * is unreachable to Line 13 on the value flow graph. However on the control
 * flow graph they are still considered as reachable, making the vf guard on
 * Line 11 -> Line 15 a true condition (should consider the infeasible branch
 * Line 11 -> Line 13) Therefore, we collect this infeasible branch condition
 * (condition on Line 11 -> Line 13, `a == b`) as an invalid condition
 * (invalidCond), and add the negation of invalidCond when computing value flow
 * guard starting from the source of the SU. In this example, we add `a != b` on
 * Line 11 -> Line 15.
 *
 * @param cur current SVFG node
 * @return invalid branch condition
 */
ProgSlice::Condition ProgSlice::computeInvalidCondFromRemovedSUVFEdge(
    const SVFGNode* cur)
{
    Set<const SVFBasicBlock*> validOutBBs; // the BBs of valid successors
    for (SVFGNode::const_iterator it = cur->OutEdgeBegin(),
                                  eit = cur->OutEdgeEnd();
         it != eit; ++it)
    {
        const SVFGEdge* edge = (*it);
    ...
    if (suVFEdgesIt != getRemovedSUVFEdges().end())
    {
        for (const auto& succ : suVFEdgesIt->second)
        {
            ...
            {
                // removed vfg node does not reside in the BBs of valid
                // successors
                const SVFBasicBlock* nodeBB = getSVFGNodeBB(cur);
                const SVFBasicBlock* succBB = getSVFGNodeBB(succ);
                clearCFCond();
                invalidCond =
                    condOr(invalidCond, ComputeIntraVFGGuard(nodeBB, succBB));
            }
        }
    }
    return invalidCond;
 ...
 */
const VFGNode* ProgSlice::isSatisfiableForPairs()
{
         ...
         ++it)
    {
        ...
        if (pathAllocator->isNegCond(*it))
            eventStack.push_back(SVFBugEvent(
                SVFBugEvent::Branch | ((((u32_t) false) << 4) & BRANCHFLAGMASK),
                tinst));
        else
            eventStack.push_back(SVFBugEvent(
                SVFBugEvent::Branch | ((((u32_t) true) << 4) & BRANCHFLAGMASK),
                tinst));
    }
}
