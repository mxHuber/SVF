...
bool SrcSnkDDA::isInAWrapper(const SVFGNode* src, CallSiteSet& csIdSet)
{
    ...
    while (!worklist.empty())
    {
             ...
             it != eit; ++it)
        {
            ...
            // assert(edge->isDirectVFGEdge() && "the edge should always be
            // direct VF");
            ...
            // (2) an intra indirect edge, we only track if the succ SVFGNode is
            // a load, which means we only track one level store-load pair . (3)
            // do not track for all other interprocedural edges.
            else
            {
