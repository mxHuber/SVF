//===- SVFBugReport.cpp -- Base class for
// statistics---------------------------------//
//
...
{
    const SVFBugEvent& sourceInstEvent =
        bugEventStack.at(bugEventStack.size() - 1);
...
void BufferOverflowBug::printBugToTerminal() const
{
    ...
    {
        SVFUtil::errs() << SVFUtil::bugMsg1("\t Full Overflow :")
                        << " accessing at : (" << GenericBug::getLoc() << ")\n";
    }
    else
    {
        SVFUtil::errs() << SVFUtil::bugMsg1("\t Partial Overflow :")
                        << " accessing at : (" << GenericBug::getLoc() << ")\n";
    }
    bugInfo << "\t\t  allocate size : [" << allocLowerBound << ", "
            << allocUpperBound << "], ";
...
{
    SVFUtil::errs() << SVFUtil::bugMsg1("\t NeverFree :")
...
cJSON* PartialLeakBug::getBugDescription() const
{
    ...
    for (auto eventIt = bugEventStack.begin(); eventIt != lastBranchEventIt;
         eventIt++)
    {
        cJSON* newBranch = cJSON_CreateObject();
        cJSON* branchLoc = cJSON_Parse((*eventIt).getEventLoc().c_str());
        if (branchLoc == nullptr)
            branchLoc = cJSON_CreateObject();

        cJSON* branchCondition =
            cJSON_CreateString((*eventIt).getEventDescription().c_str());
...
{
    SVFUtil::errs() << SVFUtil::bugMsg2("\t PartialLeak :")
                    << " memory allocation at : (" << GenericBug::getLoc()
                    << ")\n";
    ...
    for (auto eventIt = bugEventStack.begin(); eventIt != lastBranchEventIt;
         eventIt++)
    {
        SVFUtil::errs() << "\t\t  --> (" << (*eventIt).getEventLoc() << "|"
...
{
    SVFUtil::errs() << SVFUtil::bugMsg2("\t Double Free :")
                    << " memory allocation at : (" << GenericBug::getLoc()
                    << ")\n";
...
const std::string DoubleFreeBug::getBug() const
{
    std::string bugString = "";
    bugString += SVFUtil::bugMsg2("\t Double Free :") +
                 " memory allocation at : (" + GenericBug::getLoc() + ")\n";

    bugString += "\t\t double free path: \n";
    auto lastBranchEventIt = bugEventStack.end() - 1;
    for (auto eventIt = bugEventStack.begin(); eventIt != lastBranchEventIt;
         eventIt++)
    {
        bugString += "\t\t  --> (" + (*eventIt).getEventLoc() + "|" +
                     (*eventIt).getEventDescription() + ") \n";
    }
    bugString += "\n";
...
{
    SVFUtil::errs() << SVFUtil::bugMsg1("\t FileNeverClose :")
                    << " file open location at : (" << GenericBug::getLoc()
                    << ")\n";
}
...
{
    SVFUtil::errs() << SVFUtil::bugMsg2("\t PartialFileClose :")
                    << " file open location at : (" << GenericBug::getLoc()
                    << ")\n";
...
{
    SVFUtil::errs() << SVFUtil::bugMsg2("\t FullNullPtrDereference :")
                    << " dereference at : (" << GenericBug::getLoc() << ")\n";
}
...
{
    SVFUtil::errs() << SVFUtil::bugMsg2("\t PartialNullPtrDereference :")
                    << " dereference at : (" << GenericBug::getLoc() << ")\n";
}
    ...
    for (auto bugPtr : bugSet)
    {
        ...
        if (BufferOverflowBug::classof(bugPtr))
        {
            ...
            for (const SVFBugEvent& event : bugEventStack)
            {
                ...
                // Function name
                cJSON* eventFunc =
                    cJSON_CreateString(event.getFuncName().c_str());
                ...
                // Event description
                cJSON* eventDescription =
                    cJSON_CreateString(event.getEventDescription().c_str());
                cJSON_AddItemToObject(singleEvent, "Description",
                                      eventDescription);
